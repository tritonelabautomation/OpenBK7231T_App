/*
 * HT7017 Driver - Datasheet Compliant Version
...
#define HT7017_REG_RMS_I1       0x06  // Current RMS
#define HT7017_REG_RMS_U        0x08  // Voltage RMS
#define HT7017_REG_FREQ         0x09  // Frequency
#define HT7017_REG_POWER_P1     0x0A  // Active Power
...
static void HT7017_ProcessResponse(uint8_t *rx_data) {
    ...
    // 1. Calculate Checksum (Sum of DATA bytes only, then Invert)
    ...
    if (calc_chk != rx_data[3]) {
        ...
        // We continue anyway to see if data makes sense
    }

    // 2. Parse 24-bit Value (Big Endian)
...
void HT7017_AppendInformationToHTTPIndexPage(http_request_t* request) {
    ...
    strcat(request->reply, tmp);
    sprintf(tmp, "Debug: TX=%u RX=%u Err=%u<br>", g_total_tx_bytes, g_total_rx_bytes, g_chk_err);
    strcat(request->reply, tmp);
}
